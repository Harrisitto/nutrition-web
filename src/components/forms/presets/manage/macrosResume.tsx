    ...
    return (
        <div className="h-44 w-full">
            <ResponsiveContainer width="100%" height="100%">
                <RechartsBarChart data={chartData} margin={{ top: 8, right: 8, left: -10, bottom: 6 }}>
                    <XAxis dataKey="name" tick={{ fontSize: 11, fill: "#2f3b1a" }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fontSize: 10, fill: "#2f3b1a" }} axisLine={false} tickLine={false} width={28} />
                    ...
                    <Tooltip
                        formatter={(value, name) => {
                            ...
                            const pct = totalMacros > 0 ? Math.round((numeric / totalMacros) * 100) : 0;
                            return [`${numeric} g (${pct}%)`, String(name ?? t("forms:preset.fields.macro"))];
                        }}
                        contentStyle={{
                            borderRadius: "0.5rem",
...
export const MacrosResume = ({
    ...
    const presetMacros = useMemo(() => {
        ...
        if (!userWeight.data || userWeight.data < 0) return {
            ...data,
            carbsPerKg: 0,
            fatPerKg: 0,
            proteinPerKg: 0,
        ...
        <section className="mt-3 rounded-xl border border-nutrition-green/30 bg-gradient-to-br from-nutrition-green/15 via-nutrition-green/10 to-white-green/60 p-4 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
                <article className="rounded-lg border border-nutrition-green/20 bg-white-green/70 px-3 py-2">
                    ...
                    <p className="mt-1 text-xl font-bold text-dark-green">{formatMetric(presetMacros.carbsPerKg)}</p>
                    <p className="mt-1 text-xs text-text-subtitle">
                        {t("forms:preset.fields.totalCarbs", { value: formatTotal(presetMacros.carbs) })}
                    </p>
                </article>
                ...
                <article className="rounded-lg border border-nutrition-green/20 bg-white-green/70 px-3 py-2">
                    ...
                    <p className="mt-1 text-xl font-bold text-dark-green">{formatMetric(presetMacros.fatPerKg)}</p>
                    <p className="mt-1 text-xs text-text-subtitle">
                        {t("forms:preset.fields.totalFat", { value: formatTotal(presetMacros.fat) })}
                    </p>
                </article>
                ...
                <article className="rounded-lg border border-nutrition-green/20 bg-white-green/70 px-3 py-2">
                    ...
                    <p className="mt-1 text-xl font-bold text-dark-green">{formatMetric(presetMacros.proteinPerKg)}</p>
                    <p className="mt-1 text-xs text-text-subtitle">
                        {t("forms:preset.fields.totalProtein", { value: formatTotal(presetMacros.protein) })}
                    </p>
                </article>
