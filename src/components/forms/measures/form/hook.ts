...
type MeasureType = NonNullable<ReturnType<typeof useFetchAllMeasures>["data"]>[number];
...
const createDateField = (label: string) => ({
    id: fieldIds.date,
    type: "date",
    currentValue: saveDate(new Date()),
    inputProps: {
        label,
    },
    explanation: "",
...
export const Context = createContext<ReturnType<typeof useMeasuresForm> | null>(null);

export const useContextMeasuresForm = () => {
    const context = useContext(Context);
    if (!context) {
        throw new Error("useContextMeasuresForm must be used within a MeasuresFormProvider");
    }
    return context;
}
