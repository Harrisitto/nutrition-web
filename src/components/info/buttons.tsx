...
export const Button: React.FC<ButtonProps> = ({
  ...
  const variantClasses: Record<ButtonVariant, string> = {
    ...
    dark:
      "bg-dark-green text-text-light hover:bg-black-green shadow-md hover:-translate-y-0.5",
    secondary:
    ...
    instagram:
      "bg-nutrition-purple text-text-light hover:opacity-90 shadow-md",
  };
...
/* ==========================================
   Botón específico para navegación de Carrusel
   ========================================== */
...
export const CarouselNavButton: React.FC<CarouselNavButtonProps> = ({
  ...
  const posClass = isPrev ? "left-0 -translate-x-4" : "right-0 translate-x-4";

  return (
    <button
      type="button"
      aria-label={isPrev ? "Anterior" : "Siguiente"}
      className={`absolute top-1/2 -translate-y-1/2 ${posClass} bg-white p-3 rounded-full shadow-lg border border-gray-blue-200 text-text-title hover:bg-gray-blue-50 transition-all focus:outline-none focus:ring-2 focus:ring-nutrition-green z-10 ${className}`}
      {...props}
    >
      <svg
        className={`w-5 h-5 fill-current transform ${isPrev ? "rotate-180" : ""}`}
        viewBox="0 0 20 20"
      >
        <path
          fillRule="evenodd"
