import {
  ...
  useFetchAvailableClients,
  useFetchInvitedClients,
  useMutateUserInvitations,
} from "@src/services/tanstack/user/invitations";
import { useCallback, useState } from "react";
import { useTranslation } from "react-i18next";
import { ConfigurationPages } from "@src/components/header";
import {
  useFetchNutritionistUsers,
  useRemoveClient,
} from "@src/services/tanstack/user/profile";

const Invitation = ({ id, code }: { id: string; code: string }) => {
  ...
  const handleSendInvitation = useCallback(() => {
    if (message.length < 10) {
      setError(
        t("data:configuration.sections.invitations.messageTooShort") ??
  ...
  return (
    <li className="rounded-xl border border-nutrition-green/20 bg-white-green/60 p-4 shadow-sm transition-colors hover:border-nutrition-green/35">
      ...
      <p className="text-sm font-medium text-dark-green">
        {t("data:configuration.sections.invitations.writeMessage")}
      </p>
      <textarea
        value={message}
        onChange={(e) => setMessage(e.target.value)}
        placeholder={
          t("data:configuration.sections.invitations.messagePlaceholder") ??
          "Write a message..."
        }
...
const Invited = ({
  ...
  return (
    <li className="rounded-xl border border-nutrition-green/20 bg-white-green/60 p-4 shadow-sm transition-colors hover:border-nutrition-green/35">
      ...
      {message ? (
        <p className="text-sm text-dark-green/90">
          {t("data:configuration.sections.invitations.message", { message })}
        </p>
      ) : null}
      <button
    ...
    >
      <section className="rounded-2xl border border-nutrition-green/20 bg-gradient-to-br from-white to-white-green/70 p-5 shadow-md">
        <input
          ...
          placeholder={
            t("data:configuration.sections.invitations.searchPlaceholder") ??
        ...
        {query.isLoading && (
          <p className="text-sm text-text-muted">
            {t("data:configuration.sections.invitations.loading")}
          </p>
        )}
        {query.isError && (
          <p className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">
            {t("data:configuration.sections.invitations.error")}
          </p>
        )}
        {query.data && query.data.length === 0 && (
          <p className="rounded-lg border border-nutrition-green/15 bg-white-green/40 px-3 py-3 text-sm text-text-muted">
            {t("data:configuration.sections.invitations.noResults")}
          </p>
        )}
        {query.data && query.data.length > 0 && (
          <ul className="space-y-3">
            {query.data.map((client) => (
              <Invitation
                key={client.invitation_code}
                code={client.invitation_code}
                id={client.user_id}
              />
            ))}
          </ul>
  ...
  return (
    ...
    >
      <section className="rounded-2xl border border-nutrition-green/20 bg-gradient-to-br from-white to-white-green/70 p-5 shadow-md">
        ...
        {query.data && query.data.length === 0 && (
          <p className="rounded-lg border border-nutrition-green/15 bg-white-green/40 px-3 py-3 text-sm text-text-muted">
            {t("data:configuration.sections.invitations.noInvitations")}
          </p>
        )}
        {query.data && query.data.length > 0 && (
          <ul className="space-y-3">
...
const ManageClients = () => {
  ...
  return (
    <ConfigurationPages
      ...
      description={t(
        "data:configuration.sections.invitations.manageClientsDescription",
      )}
    >
      {clients.data?.map((client) => (
        <div
          key={client.user_id}
          className="rounded-xl border border-nutrition-green/20 bg-white-green/60 p-4 shadow-sm transition-colors hover:border-nutrition-green/35"
        >
          <div className="mb-2 flex items-center justify-between gap-2">
            <p className="text-xs font-semibold uppercase tracking-wide text-text-muted">
              {t("data:configuration.sections.invitations.client")}
...
export default PageInviteClient;
